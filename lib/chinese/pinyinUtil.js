...
(function (global, factory) {
  ...
  const pinyinUtil = {
    ...
    parseDict() {
      ...
      if (window.pinyin_dict_withtone) {
        ...
        if (window.pinyin_dict_notone) {
          ...
          const notone = pinyinUtil.removeTone(pinyin_dict_withtone).split(',');
          const py2hz = {}; let py; let
            hz;
    ...
    getPinyin(chinese, splitter, withtone, polyphone) {
      ...
      {
        if (withtone) console.info('pinyin_dict_notone 字典文件不支持声调！');
        if (polyphone) console.info('pinyin_dict_notone 字典文件不支持多音字！');
        var noChinese = '';
        for (var i = 0, len = chinese.length; i < len; i++) {
          const temp = chinese.charAt(i);
          var pinyin = dict.notone[temp];
          if (pinyin) { // 插入拼音
            // 空格，把noChinese作为一个词插入
            noChinese && (result.push(noChinese), noChinese = '');
            result.push(pinyin);
